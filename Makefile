...
help:
	...
	@echo "  run-http       - Run the MCP server in HTTP mode"
...
test:
	...
	@if command -v uv >/dev/null 2>&1; then \
		uv run pytest -m "not vfs_integration"; \
	elif command -v pytest >/dev/null 2>&1; then \
		pytest -m "not vfs_integration"; \
	else \
		python -m pytest -m "not vfs_integration"; \
	fi
...
run:
	@echo "Running Stage MCP server (STDIO mode)..."
	@if command -v uv >/dev/null 2>&1; then \
		PYTHONPATH=src uv run python -m chuk_mcp_stage.server; \
	else \
		PYTHONPATH=src python3 -m chuk_mcp_stage.server; \
	fi

# Run the MCP server in HTTP mode
run-http:
	@echo "Running Stage MCP server (HTTP mode)..."
	@if command -v uv >/dev/null 2>&1; then \
		PYTHONPATH=src uv run python -m chuk_mcp_stage.server http; \
	else \
		PYTHONPATH=src python3 -m chuk_mcp_stage.server http; \
	fi
...
docker-build:
	...
	docker build -t chuk-mcp-stage:latest .
	@echo "Docker image built: chuk-mcp-stage:latest"

# Run Docker container
docker-run:
	@echo "Running Docker container..."
	docker run --rm -p 8000:8000 chuk-mcp-stage:latest
...
docker-stop:
	...
	@docker ps -q --filter "ancestor=chuk-mcp-stage:latest" | xargs -r docker stop

# Remove Docker image
docker-clean:
	@echo "Removing Docker image..."
	@docker rmi chuk-mcp-stage:latest 2>/dev/null || true
...
publish-test: build
	...
	echo ""; \
	echo "Install with:"; \
	echo "  pip install --index-url https://test.pypi.org/simple/ chuk-mcp-stage==$$version"
...
publish-manual: build
	...
	echo "Install with: pip install chuk-mcp-stage==$$version"
